...
v2.array = function array(data) {
    ...
    return v2.empty(data) ? [] : (typeof data.shift == 'undefined' ? [data] : data);
};
/**
        ...
        (element = elements[j]); j++) {
            ...
            for (k = 0; k < attributes.length; k++) {
                ...
                if (element.hasAttribute) {
                    if (!element.hasAttribute(attr) || (value !== true && element.getAttribute(attr) !== value)) {
                        continue elementLoop;
                    }
                } else {
                    // IE6 makes it stink...
                    attr = attr === 'for' ? 'htmlFor' : attr;
                    if (typeof element[attr] === 'undefined' || (value !== true && element[attr] !== value)) {
                        continue elementLoop;
                    }
                }
            }
...
v2.Element = {
     ...
     */
    visible: function(el) {
        return v2.Element.computedStyle(el, 'display') != 'none' && v2.Element.computedStyle(el, 'visibility') != 'hidden' && (el.parentNode === null || el.parentNode.nodeType != 1 || v2.Element.visible(el.parentNode));
    },
    /**
...
v2.Object.extend(Function.prototype, /** @scope Function.prototype */ {
    ...
    bind: function(me) {
        ...
        fn.__cc[me.__id] = fn.__cc[me.__id] ||
        function() {
            try {
...
v2.CompositeFormItem = Base.extend( /** @scope v2.CompositeFormItem.prototype */ {
    type: 'generic',
    // Makes for easier type checking
    /**
...
/*if (this.__passExceptions()) {
      return true;
        ...
        fn = fn || 'test';
        for (i = 0;
        (validator = this.__validators[i]); i++) {
            if (validator[fn]()) {
                valid++;
            } else {
        ...
        return this.passOnAny() && valid > 0 || !this.passOnAny() && valid === this.__validators.length;
    },
    /**
...
v2.InputElement = Base.extend( /** @scope v2.InputElement.prototype */ {
    ...
    monitor: function(eventHandler) {
        ...
        for (i = 0;
        (element = this.__elements[i]); i++) {
            for (j = 0;
            (event = this.__events[j]); j++) {
    ...
    get: function(idNameEl) {
        ...
        var element = v2.$(idNameEl),
            id, field, elements, selector, className, pattern;
...
v2.Message = Base.extend( /** @scope v2.Message */ {
    ...
    toString: function() {
        ...
        for (var i = 0, param = null;
        (param = this.params[i]); i++) {
...
v2.Validator = Base.extend( /** @scope v2.Validator.prototype */ {
    ...
    test: function(field, params, invert) {
        ...
        var result = (this.acceptEmpty && value === '') || this.__test(field, value, params);
...
v2.Field = v2.CompositeFormItem.extend( /** @scope v2.Field.prototype */ {
    ...
    // If a field should be validated even if it's not
    // visible
    instant: false,
    // If validations should be run instantly
    instantWhenValidated: true,
    // If validations should be run instantly after
    // the validation has been run once
    type: 'field',
    // Identifier, makes for easier type checking
    /**
...
v2.Form = v2.CompositeFormItem.extend( /** @scope v2.Form.prototype */ {
    ...
    validate: function() {
        ...
        this.__activeButton = null;
        if (buttons.length > 0 && buttons.indexOf(button) < 0) {
            return true;
        }
        var valid = this.base();
        if (!valid && event) {
...
v2.dsl = /** @scope v2.dsl */
{
    ...
    __validateTemplate: function(conditional) {
        return function() {
            ...
            for (var i = 0, component;
            (component = arguments[i]); i++) {
    ...
    addValidator: function(validator, params, and) {
        if (typeof and !== "undefined") {
            if (this.__and !== null && this.__and !== and) {
                throw new Error("Field previously set up with " + (this.__and ? "AND" : "OR") + ", unable to shift");
            }
            this.__and = and;
...
v2.dsl.Form = Base.extend( /** @scope v2.dsl.Form.prototype */ {
    ...
    on: function() {
        ...
        for (i = 0; i < arguments.length; i++) {
            ...
            this.item.addButton(button.tagName.toLowerCase() !== 'input' ? button.getElementsByTagName('input')[0] : button);
        }
    }
});
/**
...
/* Check if parameters reference elements
    ...
    applyValidators: function(validators, field, message) {
        ...
        for (var i = 0, validator, fv;
        (validator = validators[i]); i++) {
...
v2.html.Form = Base.extend( /** @scope v2.html.prototype */ {
    ...
    parseElement: function(container, collection, validators) {
        ...
        (element = elements[i]); i++) {
            ...
            if (validators.length === 0 && (/^\s*$/.test(element.className) || this.__parsed[element.id || element.name])) {
                continue;
            }
            tagName = element.tagName.toLowerCase();
            // Look for button
            if (tagName === 'input' && v2.Element.hasClassName(element, v2.Form.actionButtonClass)) {
...
v2.ErrorReporting = {
    displayErrors: -1,
    // How many error messages to display per item
    // Default is -1 = all messages
    positionErrorsAbove: true,
    // Whether to display error messages above item
    // (true - default) or below (false)
    failureClass: 'error',
    // Class name to append to the parent element
    // when validation fails
    successClass: '',
    // Class name to append to the parent element
    // when validation succeeds
    messagesClass: 'messages',
    // Class name used on error lists
    /**
    ...
    onFailure: function() {
        ...
        var str = '',
            i, error, validation, listElement;
        var used = [];
    ...
    __getId: function() {
        ...
        var value = ((parent.id || parent.name || className.replace(this.failureClass, '')) + '_' + this.failureClass).replace(' ', '_').replace(/_+/, '_');
        return value;
    }
};
/**
...
v2.Object.extend(v2.Form.prototype, /** @scope v2.Form.prototype */ {
    ...
    // scrolling to the first field when validation
    // fails
    /**
